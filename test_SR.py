...
tf.app.flags.DEFINE_integer('scale', 3, '')
...
def Test_Image_Processing_txt(image_size):
    contents = []
        ...
        for file in files:
            if os.path.splitext(file)[1].lower() == '.txt':
                ...
                while True:
                    ...
                    if x != width and y != height:
                        for i in range(3):
                            image[image_size - y, x, i] = (float(line[2 + i]) * 255.0)
                filein.close()
...
if __name__ == '__main__':
    ...
    # test_image_list = Test_Image_Processing_txt(image_size) # if Normal Image
